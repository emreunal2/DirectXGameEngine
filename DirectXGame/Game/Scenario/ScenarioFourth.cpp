...
void ScenarioFourth::generateScenario()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		float sizeX = static_cast<float>((std::rand() % 3) + 1);
		float sizeY = static_cast<float>((std::rand() % 3) + 1);
		float sizeZ = static_cast<float>((std::rand() % 3) + 1);
		float posX = static_cast<float>((std::rand() % 90) - 45);
		float posY = static_cast<float>((std::rand() % 30) - 15);
		float posZ = static_cast<float>((std::rand() % 90) - 45);
		CreateStaticCube(sizeX, sizeY, sizeZ, posX, posY, posZ);
	}
}
...
void ScenarioFourth::CreateMovingSphere(f32 radius, f32 posx, f32 posy, f32 posz, f32 dirx, f32 diry, f32 dirz, f32 mass, MaterialType type)
{
...
void ScenarioFourth::CreateStaticSphere(f32 radius, f32 lenght, f32 posx, f32 posy, f32 posz)
{
...
void ScenarioFourth::CreateStaticCube(f32 sizex, f32 sizey, f32 sizez, f32 posx, f32 posy, f32 posz)
{
